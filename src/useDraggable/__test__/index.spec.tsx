...
import { defineComponent, CSSProperties } from '@vue/runtime-dom';
    ...
    test('starting drag', async () => {
        ...
        const wrapper = mount({
            ...
            render() {
                return (
                    <>
                        <h1 ref={(ele: HTMLElement) => { targetRef.value = ele }} > click </h1>
                    </>);
            },
        });
...
const Consumer = defineComponent(
    {
        ...
        setup(props: { useDraggableOption: Parameters<typeof useDraggable>[0], style: CSSProperties }) {
            ...
            ] = useDraggable(props.useDraggableOption);
            return () => (
                <div
                    class='container'
                    ref={targetRef}
...
async function setup(props: { useDraggableOption: Parameters<typeof useDraggable>[0], style?: CSSProperties } = { style: {}, useDraggableOption: {} }) {
    const wrapper = mount(<Consumer useDraggableOption={props.useDraggableOption} style={props.style} />);
    async function drag({
    ...
    async function beginDrag(start, touch = false) {
        ...
        if (touch) {
            let ev = new TouchEvent("touchstart", {
                bubbles: true,
                cancelable: true,
