...
export default function useDraggable(config: {
...
} = { controlStyle: true }): [Ref<ElementType>, Ref<ElementType>, { getTargetProps: () => HTMLAttributes, dragging: Ref<boolean>, delta: Ref<{ x: number, y: number }>, resetState: () => void }] {
        ...
        function startDragging(event) {
            ...
            if (viewport || rectLimits) {
                const {
                    ...
                    width,
                    height
                } = targetEleRef.value.getBoundingClientRect();
    ...
    watchEffect(() => {
        targetEleRef.value && (targetEleRef.value.style.transform = `translate(${delta.value.x}px, ${delta.value.y}px)`);
    }, { flush: 'post' })
